...
def axis_angle_to_quaternion(axis_angle):
    return roma.rotvec_to_unitquat(axis_angle)
...
class RotateAxisAngle:
    def __init__(self, angle, axis="X", degrees=True):
        ...
        self.rotvec = torch.tensor([x * angle_rad for x in v], dtype=torch.float32)

    def transform_points(self, points):
        R = roma.rotvec_to_rotmat(self.rotvec).to(points.device)
        return points @ R.T
#----------------------------------
